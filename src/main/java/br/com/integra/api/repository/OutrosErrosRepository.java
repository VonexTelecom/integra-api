...
public class OutrosErrosRepository {
	 ...
	 * @param tipoEstatistica(tipo de estatística(chamadas_ddd) a ser feita na
	 *                             query)
	...
	// Caso o tempo inicial seja customizado(diferente de 00:00:00) e o tempo final
	// não seja informado pelo service esse método recolhe o todos
	// os ddds a partir da data inicial passada pelo front até a 23:59
	public List<OutrosErros> findtipoOutrosErrosInicial(LocalDate date, EstatisticaFilter filter, Long clienteId) {
		...
		String dataInicialFormatada = DateUtils
				.formatarData(filter.getDataInicial().toInstant().atZone(ZoneId.systemDefault()).toLocalDateTime());
		...
		if (filter.getModalidade().size() == 0 && filter.getDiscador().size() == 0 && filter.getOperadora().size() == 0
				&& filter.getUnidadeAtendimento().size() == 0) {
		...
		String sql = FiltroEstatisticaUtils.criarQueryOutrosErros(nomeDaTabelaData, "", filter, clienteId, null, null,
				dataInicialFormatada, dataFinalFormatada);

		// conversor da lista dos resultados da query em lista de entidades do spring
		List<OutrosErros> listaOutrosErros = namedJdbcTemplate.query(sql,
				new RowMapperResultSetExtractor<OutrosErros>(new OutrosErrosRowMapper()));
	...
	public List<OutrosErros> findtipoOutrosErrosFinal(LocalDate date, EstatisticaFilter filter, Long clienteId) {
		...
		if (filter.getDataInicial() != null && filter.getDataInicial().toInstant().atZone(ZoneId.systemDefault())
				.toLocalTime().compareTo(LocalTime.of(8, 00)) == 0) {
		...
		String dataFinalFormatada = DateUtils
				.formatarData(filter.getDataFinal().toInstant().atZone(ZoneId.systemDefault()).toLocalDateTime());
	...
	// Método para query sem data inicial e data final (busca todos os dados da
	// tabela inteira)
	public List<OutrosErros> findtipoOutrosErros(LocalDate date, EstatisticaFilter filter, Long clienteId) {
